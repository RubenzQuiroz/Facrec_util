...
SELECT
    ...
    CS.COD_DISTRIBUIDOR,
  	...
  	CS.ID_TIPO_SOCIO,
    CE.NOMBRE ESTADO,
    CCOM.VALOR PORCENTAJE,
    CCOM.VALOR_FACIAL
FROM AFC_SOCIO CS
INNER JOIN AFG_GRP_SOCIO GS on GS.ID_GRP_SOCIO = CS.ID_GRP_SOCIO
INNER JOIN AFC_ESTADO CE on CE.ID_ESTADO = GS.ID_ESTADO
INNER JOIN AFC_DEUDOR CD on CD.ID_DEUDOR = GS.ID_DEUDOR
INNER JOIN AFC_CONFIGURACION CC on CC.ID_CONFIGURACION = GS.ID_CONFIGURACION
INNER JOIN AFC_COMISION CCOM on CCOM.ID_COMISION = CC.ID_COMISION
WHERE CS.COD_DISTRIBUIDOR = '200408';
